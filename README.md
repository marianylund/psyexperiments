...
This is a repository for running experiments on the web using jspsych library. Currently running at http://158.39.75.25/
...
You can work on this project in Github Codespaces. Click the green Code button -> Codespaces and select a codespace or create a new one to get started.

When the editor is opened and ready you can open the frontend folder on the right side and select an experiment to edit (experiment.js for instance) or clone an experiment file to create a new experiment.

To publish a new version you can select source control on the left side, write what you changed and commit the changes and sync them to github. When you sync your changes to github all servers with autoupdate will update after a few minutes.

To test the experiment locally first (recommended) open a terminal in the codespace and navigate to the frontend folder (`cd frontend`) and then start an experiment `npm run start experiment`. Replace experiment with the filename of the experiment you want to run. This should give you a popup to open the experiment in a new tab. 
...
Server has read-access to this repository to get the code, build and serve it. Currently, `158.39.75.25` should be running at all times.
...
2. In the console of your choice run `ssh -i ~/.ssh/nrec ubuntu@158.39.75.25`, where `~/.ssh/nrec` is the path to the SSH key on your machine.
3. (Ignore for now) If you have set up that only logged-in users can join the server you´ll need to alternatively run `ssh -J username@login.uio.no -i ~/.ssh/nrec ubuntu@158.39.75.25`, where `username` is your UiO username.
